...
const NavLinks = ({ onClick }: { onClick?: () => void }) => {
  ...
  const router = useRouter();
    ...
    const itemsToShow = navItems.filter((item) => item.path !== "/admin" || (user?.role?.toLowerCase() === "admin"));
        ...
        {itemsToShow.map(({ path, label }) => {
          ...
          return (
              <button
                  ...
                  className={`
              relative px-4 py-2 mx-1 my-1 rounded-xl transition font-semibold group
              ${
                      isActive
